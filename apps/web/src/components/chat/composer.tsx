...
import { ImagePlus, Loader2, SendHorizontal, X, Edit2 } from "lucide-react";
...
export function Composer({ conversationId }: { conversationId: string }) {
	...
	const content = editingMessage ? editContent : draft;
	...
	const handleChange = useCallback(
		...
		[conversationId, setDraft, autoResize, emitTypingStart, emitTypingStop, editingMessage],
	);
	...
	}, [setEditingMessage, draft]);

	const handleSend = useCallback(() => {
		void (async () => {
			const trimmed = content.trim();
			if (!trimmed && !selectedImageFile) return;
			if (sendMessage.isPending || editMessage.isPending || isImageMessageFlowPending) return;
	...
	useEffect(() => {
		autoResize();
	}, [conversationId, autoResize, editingMessage]);

	const canSend = (!!content.trim() || !!selectedImageFile) && !sendMessage.isPending && !editMessage.isPending && !isImageMessageFlowPending;

	return (
		<div className="shrink-0 px-4 py-3 bg-background">
			{editingMessage && (
				<div className="mb-2 flex items-center justify-between rounded-xl bg-primary/10 px-3 py-2 text-primary ring-1 ring-primary/20">
					...
					<button
						...
						className="rounded-md p-1 hover:bg-primary/20 transition-colors focus:outline-none"
					>
						<X className="h-4 w-4" />
					</button>
				</div>
			...
			{selectedImagePreviewUrl && !editingMessage && (
